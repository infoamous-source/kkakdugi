...
export default function MarketResearchReportView({ report, keyword, onClose }: Props) {
  ...
  const handleExportPDF = async () => {
    ...
    setExporting(true);
    try {
      await exportToPdf(reportRef.current, `시장리서치_${keyword}_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto">
      ...
      <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-10">
        ...
        <button onClick={handleExportPDF} disabled={exporting}
          className="flex items-center gap-2 px-5 py-2.5 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 shadow-lg disabled:opacity-50">
