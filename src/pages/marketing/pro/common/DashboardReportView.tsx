...
export default function DashboardReportView({ analysis, entries, goalROAS, onClose }: Props) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`마케팅대시보드_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 overflow-y-auto">
      <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-10">
        ...
        <button onClick={handleExportPDF}
          className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 shadow-lg">
