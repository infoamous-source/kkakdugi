...
import {
  type FastfoodScreen,
  type FastfoodCartItem,
  type MenuCategory,
  FASTFOOD_SCREEN_ORDER,
  calculateItemPrice,
  calculateCartTotal,
} from './data';
...
export default function FastfoodKkakdugi({ onClose, onComplete }: KkakdugiComponentProps) {
  ...
  // ── Cart ────────────────────────────────────────────────────────────────────
  const [cart, setCart] = useState<FastfoodCartItem[]>([]);
  ...
  // ── Screen flow indices ──────────────────────────────────────────────────────
  const currentStepIndex = FASTFOOD_SCREEN_ORDER.indexOf(screen);
  const totalSteps = FASTFOOD_SCREEN_ORDER.length;

  // ── Helper messages ──────────────────────────────────────────────────────────
  const helperMessages: Record<FastfoodScreen, string> = useMemo(() => ({
    ...
    options:      t('kkakdugi.helper.options',      '사이즈업, 소스, 토핑 옵션을 선택하고 담기를 눌러주세요.'),
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 kiosk-overlay">
      ...
      >
        ...
        <div
          ...
          style={{ backgroundColor: '#2C0000', borderBottom: '1px solid #4A0000' }}
        >
          <span className="text-xs font-medium" style={{ color: '#FFCDD2', minWidth: 60 }}>
            {t('kkakdugi.step', '단계')} {currentStepIndex + 1} / {totalSteps}
          </span>
          <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ backgroundColor: '#4A0000' }}>
            <div
        ...
        {/* ── Screen content ── */}
        <div className="flex-1 overflow-hidden relative" style={{ backgroundColor: '#FFF9F0' }}>
