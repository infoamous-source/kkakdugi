...
import { formatPrice, calculateCartTotal, recommendItems, type CafeCartItem, type MenuCategory } from '../data';
    ...
    <div className="flex flex-col h-full" style={{ backgroundColor: '#FFF8F0' }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div
          ...
          style={{ borderBottom: '1px solid #E8D5C0', backgroundColor: 'white' }}
        >
          <span className="flex-1 text-xs" style={{ color: '#9E7E6A' }}>
            {t('kkakdugi.screens.menu.cartHeader.menu', '메뉴')}
          </span>
          <span className="text-xs mr-8" style={{ color: '#9E7E6A' }}>
            {t('kkakdugi.screens.menu.cartHeader.quantity', '수량')}
          </span>
          <span className="text-xs" style={{ color: '#9E7E6A', minWidth: 64, textAlign: 'right' }}>
        ...
        {cart.map((cartItem, i) => (
          <div
            ...
            style={{ borderBottom: '1px solid #F5EDE2', backgroundColor: 'white' }}
          >
            <button
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium truncate" style={{ color: '#2C1A0E', fontSize: 13 }}>
                {cartItem.menuItem.nameKey}
              </p>
              {cartItem.selectedOptions.length > 0 && (
                <p className="text-xs truncate" style={{ color: '#9E7E6A' }}>
            ...
            <div className="flex items-center gap-1.5 mx-3">
              <button
                ...
                style={{ width: 24, height: 24, border: '1px solid #E8D5C0', color: '#6B4E35', backgroundColor: 'white' }}
              >
                <Minus size={10} />
              </button>
              <span
                className="font-bold text-center"
                style={{ color: '#2C1A0E', fontSize: 13, minWidth: 20 }}
              >
                {cartItem.quantity}
              </span>
              <button
                onClick={() => onUpdateQuantity(i, 1)}
                className="flex items-center justify-center rounded"
                style={{ width: 24, height: 24, border: '1px solid #9C6B3C', color: '#9C6B3C', backgroundColor: 'white' }}
              >
                <Plus size={10} />
              </button>
            </div>
            <span
              className="font-bold"
              style={{ color: '#2C1A0E', fontSize: 13, minWidth: 64, textAlign: 'right' }}
            >
        ...
        <div className="px-4 py-2">
          <p className="text-xs" style={{ color: '#DC2626' }}>
            ※ {t('kkakdugi.screens.orderConfirm.noDisposableCup', '1회용컵은 매장에서 사용할 수 없습니다')}
          </p>
        </div>

        {/* Recommend section */}
        <div className="px-4 pt-3 pb-2">
          <p className="font-bold mb-2.5" style={{ color: '#2C1A0E', fontSize: 13 }}>
          ...
          <div className="flex gap-3">
            {recommendItems.map((rec) => (
              <button
                ...
                style={{ border: '1px solid #E8D5C0', backgroundColor: 'white' }}
              >
                <div
                ...
                <div className="p-2 text-center">
                  <p className="text-xs font-medium truncate" style={{ color: '#2C1A0E', fontSize: 11 }}>
                    {rec.nameKey}
                  </p>
                  <p className="font-bold" style={{ color: '#9C6B3C', fontSize: 11 }}>
        ...
        >
          <div className="flex justify-between mb-2">
            <span style={{ color: '#6B4E35', fontSize: 13 }}>
              {t('kkakdugi.screens.orderConfirm.totalQuantity', '총 수량')}
            </span>
            <span className="font-bold" style={{ color: '#2C1A0E', fontSize: 13 }}>
              {count}{t('kkakdugi.screens.cart.itemUnit', '개')}
            </span>
          </div>
          <div className="flex justify-between items-center pt-2" style={{ borderTop: '1px solid #F5EDE2' }}>
            <span className="font-bold" style={{ color: '#2C1A0E', fontSize: 14 }}>
              {t('kkakdugi.screens.orderConfirm.totalAmount', '합계')}
            </span>
            <span className="font-bold" style={{ color: '#9C6B3C', fontSize: 22 }}>
              {formatPrice(total)}원
            </span>
          </div>
        </div>
      ...
      <div
        ...
        style={{ borderTop: '2px solid #E8D5C0', backgroundColor: 'white' }}
      >
        <button
          onClick={onBack}
        ...
        >
          <span style={{ color: '#C89B3C' }}>{formatPrice(total)}원 </span>
