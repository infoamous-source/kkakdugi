...
import { type MenuCategory, type CafeCartItem, categories, menuItems, formatPrice } from '../data';
    ...
    <div className="flex flex-col h-full" style={{ backgroundColor: '#FFF8F0' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div
          ...
          style={{ width: 72, backgroundColor: '#2C1A0E', borderRight: '1px solid #4A3020' }}
        >
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                style={{
                  minHeight: 64,
                  backgroundColor: isActive ? '#9C6B3C' : 'transparent',
                  color: isActive ? 'white' : '#9E7E6A',
            ...
            {filtered.map((item) => {
              ...
              return (
                <button
                  ...
                  style={{ backgroundColor: 'white', border: '1px solid #E8D5C0', boxShadow: '0 1px 3px rgba(61,43,31,0.08)' }}
                >
                  {/* Image placeholder */}
                  <div
                  ...
                  >
                    {/* Item name initial letters as visual */}
                    <span
                    ...
                    {item.popular && (
                      <span
                        ...
                        style={{ backgroundColor: '#C89B3C', color: 'white', fontSize: 9 }}
                      >
                        인기
                      </span>
                    )}
                    {inCart && (
                      <span
                        ...
                        style={{ backgroundColor: '#3D2B1F', color: 'white', fontSize: 10 }}
                      >
                        {inCart.quantity}
                      </span>
                    )}
                  ...
                  <div className="p-2">
                    <p
                      ...
                      style={{ color: '#2C1A0E', fontSize: 11 }}
                    >
                      {item.nameKey}
                    </p>
                    <p
                      className="font-bold"
                      style={{ color: '#9C6B3C', fontSize: 12 }}
                    >
      ...
      >
        {/* Cart items (compact) */}
        {cart.length > 0 && (
          <div style={{ maxHeight: 90, overflowY: 'auto', borderBottom: '1px solid #E8D5C0' }}>
            {cart.map((item, i) => (
              <div
                key={i}
                className="flex items-center px-3 py-1.5 gap-2"
                style={{ borderBottom: i < cart.length - 1 ? '1px solid #F5EDE2' : 'none' }}
              >
                <button
                  ...
                  style={{ width: 18, height: 18, backgroundColor: '#F5EDE2', color: '#9E7E6A' }}
                >
                  <X size={10} />
                </button>
                <span
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    style={{ width: 20, height: 20, backgroundColor: '#F5EDE2', color: '#6B4E35' }}
                  >
                    <Minus size={10} />
                  </button>
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#2C1A0E', minWidth: 16, textAlign: 'center' }}>
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => onUpdateQuantity(i, 1)}
                    className="flex items-center justify-center rounded transition-colors"
                    style={{ width: 20, height: 20, backgroundColor: '#9C6B3C', color: 'white' }}
                  >
                    <Plus size={10} />
                  </button>
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#9C6B3C', minWidth: 52, textAlign: 'right' }}>
